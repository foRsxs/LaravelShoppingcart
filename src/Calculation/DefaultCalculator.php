...
class DefaultCalculator implements Calculator
{
    public static function getAttribute(string $attribute, CartItem $cartItem)
    {
        ...
        switch ($attribute) {
            case 'discount':
                return !empty($cartItem->discountAmount) ? $cartItem->getDiscountAmount() :  $cartItem->price * ($cartItem->getDiscountRate() / 100);
            case 'tax':
            ...
            case 'priceTarget':
                return !empty($cartItem->qty) ? round(($cartItem->priceTotal - $cartItem->discountTotal) / $cartItem->qty, $decimals) : 0;
            case 'taxTotal':
